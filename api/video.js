...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .url-row button{background:linear-gradient(135deg,#6366f1,#a855f7);border:none;color:white;padding:10px 18px;border-radius:8px;font-weight:600;cursor:pointer;font-size:13px;transition:all .15s}
    .url-row button:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(99,102,241,0.4)}
    .copied{color:#22c55e;font-size:12px;margin-top:6px;opacity:0;transition:opacity .3s}
...
<body>
  <a href="/" class="logo animate-in"><div class="logo-icon">V</div><span class="logo-text">VidRip</span></a>
  <div class="player-container animate-in">
    <video controls autoplay playsinline><source src="${videoUrl}" type="${mimeType}">Your browser does not support the video tag.</video>
    <div class="info">
      <h2>🎬 ${title}</h2>
      <p>Share this link anywhere — Discord, Twitter, anywhere.</p>
      <div class="url-box">
        <label>🔗 Share URL</label>
        <div class="url-row"><input type="text" id="shareUrl" value="${shareUrl}" readonly><button onclick="copy('shareUrl','c1')">Copy</button></div>
        <div class="copied" id="c1">✓ Copied!</div>
      </div>
      <div class="url-box" style="margin-top:12px">
        <label>📺 Direct Video URL</label>
        <div class="url-row"><input type="text" id="directUrl" value="${videoUrl}" readonly><button onclick="copy('directUrl','c2')">Copy</button></div>
        <div class="copied" id="c2">✓ Copied!</div>
      </div>
    </div>
  </div>
  <div class="footer animate-in">Powered by <a href="/">VidRip</a></div>
  <script>async function copy(id,tid){await navigator.clipboard.writeText(document.getElementById(id).value);const el=document.getElementById(tid);el.classList.add('show');setTimeout(()=>el.classList.remove('show'),2000)}</script>
</body>
</html>`);
  } catch (err) {
    console.error('Video page error:', err);
    res.status(500).send('Error loading video: ' + err.message);
  }
};
